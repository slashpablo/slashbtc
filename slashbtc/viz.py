...
# %% auto #0
__all__ = ['block_feerates', 'feerate_buckets', 'block_summary', 'feerate_trend', 'fee_distribution_chart', 'block_card',
           'blocks_view']
...
from .fee import FeeBlock, fee_histogram, fee_summary
...
def block_feerates(
    ...
    fee_block = FeeBlock.from_rpc_block(blk)
    rows = [(tx.vsize_vb, tx.fee_sat_vb) for tx in fee_block.txs]
    result = (blk, rows)
...
def fee_distribution_chart(
    ...
    """Render a mempool.space-style fee distribution panel for one compact fee block."""
    summary = fee_summary(block)
    buckets = fee_histogram(block, max_sat_vb=max_sat_vb, include_empty=True)
